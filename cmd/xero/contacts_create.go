...
var contactsCreateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if cmd.Flags().Changed("first-name") {
			if firstName == "" {
				return validationError("--first-name cannot be empty")
			}
			contact["FirstName"] = firstName
		}
		if cmd.Flags().Changed("last-name") {
			if lastName == "" {
				return validationError("--last-name cannot be empty")
			}
		...
		if !hasKey(contact, "Name") && !hasKey(contact, "FirstName") && !hasKey(contact, "LastName") {
			return validationError("--name is required (or provide Name/FirstName/LastName in --body)")
		}
...
func init() {
	...
	addStructuredInputFlags(contactsCreateCmd, "Raw JSON object of contact attributes")
